...
*   find和locate都是查询系统中文件位置的，grep是查询文件内容的
*   find通过遍历查询，结果精确但消耗大量系统资源和时间
*   locate是通过创建数据库保存文件位置，在查询数据库的一种快速索引方式，节省资源但精确度差
...
# 格式
zip 包名（生成的） 压缩对象 # 压缩格式（包名后缀为.zip）
	-r # 压缩目录时，递归压缩
unzip 包名 # 解压缩（默认当前目录）
	-d # 指定解压缩目录
unzip 包名 -d 目录
...
# 归档并压缩
tar -zcvf 包名 fiels --> *.tar.gz # 归档后使用gzip压缩
	-z # 调用gzip进行压缩
